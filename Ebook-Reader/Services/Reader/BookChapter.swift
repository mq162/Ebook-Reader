...
class BookChapter {
    ...
    /// 字体
    lazy var fontName = ReaderConfig.fontName
    /// 文字颜色
    lazy var textColorHex = ReaderConfig.textColorHex
    /// 文字大小
    lazy var textSizeMultiplier = ReaderConfig.textSizeMultiplier
    /// 章节页列表
    lazy var pageList = [BookPage]()
    /// 章节标题
    var title: String?
    /// 章节索引
    ...
    func page(at index: Int) -> BookPage? {
    ...
    func page(in loction: Int) -> BookPage? {
    ...
    func htmlAttributedString(with htmlData: Data) -> NSMutableAttributedString {
        ...
        let options: [String : Any] = [
            ...
            DTDefaultFontName: ReaderConfig.fontName,
            DTMaxImageSize: ReaderConfig.pageSzie,
            NSTextSizeMultiplierDocumentOption: (CGFloat(textSizeMultiplier) / 10.0),
            DTDefaultLineHeightMultiplier: ReaderConfig.lineHeightMultiple,
            DTDefaultLinkColor: ReaderConfig.linkColorHex,
            DTDefaultTextColor: UIColor(hexStr: textColorHex!),
            DTDefaultFontSize: ReaderConfig.defaultTextSize
        ]
        ...
        tempHtmlString?.enumerateAttributes(in: NSMakeRange(0, htmlString.length), options: [.longestEffectiveRangeNotRequired]) { (value: [NSAttributedString.Key : Any], range, stop) in
            ...
            if let paragraphStyle = value[.paragraphStyle] as? NSParagraphStyle {
                ...
                mutableStyle.paragraphSpacing = ReaderConfig.paragraphSpacing
                mutableStyle.lineSpacing = ReaderConfig.lineSpacing
                htmlString.addAttribute(.paragraphStyle, value: mutableStyle, range: range)
            }
            
            // 字体调整
            if let value = value[.font] as? UIFont {
                if value.fontName != ReaderConfig.fontName {
                    let font = UIFont.init(name: ReaderConfig.fontName, size: value.pointSize) ?? value
    ...
    func updateTextColorHex(_ textColorHex: String) {
        ...
        let textColor = UIColor(hexStr: textColorHex)
    ...
    func pagination(withHtmlAttributedText htmlString: NSMutableAttributedString) {
        ...
        if htmlString.length == 0 {
            // 空白章节
            let pageModel = BookPage.bookPage(withPageIdx: 0, chapterIdx: self.chapterIdx)
        ...
        let textRect = CGRect.init(origin: CGPoint.zero, size: ReaderConfig.pageSzie)
        ...
        while pageOffset <= htmlString.length && pageOffset != 0 {
            ...
            if textContent.count > 0 {
                let pageModel = BookPage.bookPage(withPageIdx: pageCount, chapterIdx: self.chapterIdx)
